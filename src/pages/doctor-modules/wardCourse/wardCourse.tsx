...
const CourseInTheWard = () => {
  ...
  const isDateValid = formData.date.trim() !== "" && formData.date <= todayStr;
  ...
  const handleAdd = () => {
    setFormData({ date: todayStr, courseText: "", showInCF4: false });
    setIsEditing(false);
  ...
  const confirmDelete = () => {
    if (selectedRecordId) {
      const updatedRecords = records.filter((r) => r.id !== selectedRecordId);
  ...
  const saveRecord = () => {
    ...
    if (isEditing && selectedRecordId) {
      setRecords((prev) =>
        prev.map((r) =>
          r.id === selectedRecordId ? { ...r, ...formData, entryBy: r.entryBy } : r
        )
      );
    } else {
                ...
                <div className="d-flex flex-column flex-grow-1 mb-4">
                    ...
                    {(!isMobile || hasRecords) && (
                      ...
                      >
                        ...
                        >
                          ...
                          <span className="d-none d-sm-inline">Summ</span>
                        </button>
                      </div>
                    )}
                  </div>
                  ...
                  >
                    <div className="table-responsive flex-grow-1 bg-white p-0">
                      <table className="table ward-table align-middle mb-0">

                        {/* ✅ CHANGE 1: clean thead — no inline Bootstrap padding/alignment classes */}
                        <thead>
                          <tr>
                            <th className="ward-th">Date</th>
                            <th className="ward-th">Course In the Ward</th>
                            <th className="ward-th">Entry by</th>
                            <th className="ward-th ward-th-center">Show in CF4</th>
                          </tr>
                        </thead>
                        ...
                        <tbody>
                          {paginatedRecords.length === 0 ? (
                            ...
                            // ✅ CHANGE 2: td tags use ward-td / ward-td-center — no inline Bootstrap padding
                            paginatedRecords.map((record) => (
                              ...
                              >
                                <td className="ward-td">{record.date}</td>
                                <td className="ward-td">{record.courseText}</td>
                                <td className="ward-td">{record.entryBy}</td>
                                <td className="ward-td ward-td-center">
                                  {record.showInCF4 && (
                                    <i
            ...
            >
              ...
              >
                ...
                >
                  ...
                  {isEditing ? "EDIT COURSE IN THE WARD" : "ADD COURSE IN THE WARD"}
                </h4>
                <button
                  type="button"
              ...
              <div className="modal-body p-3 p-md-4 bg-white">
                <div className="mb-3">
                  <label
                    ...
                    style={{ color: "var(--primary, #0f763f)", fontSize: "0.85rem" }}
                  >
                  ...
                  <input
                    type="date"
                    max={todayStr}
                    className={`form-control form-control-sm rounded-1 shadow-none ${
                      !isDateValid ? "is-invalid" : ""
                    }`}
                    style={{
                      borderColor: !isDateValid ? "#dc3545" : "var(--primary, #0f763f)",
                      maxWidth: "200px",
                    }}
                    value={formData.date}
                ...
                <div className="mb-1">
                  ...
                  >
                    <span>
                      Course In the Ward <span className="text-danger">*</span>
                    </span>
                  </label>
                  <textarea
                    className={`form-control rounded-1 shadow-none ${
                      !isTextValid && formData.courseText.length > 0 ? "is-invalid" : ""
                    }`}
                    rows={6}
                    maxLength={255}
                    style={{
                      borderColor:
                        !isTextValid && formData.courseText.length > 0
                          ? "#dc3545"
                          : "var(--primary, #0f763f)",
                      resize: "none",
                      fontSize: "0.9rem",
                    ...
                    onChange={(e) => setFormData({ ...formData, courseText: e.target.value })}
                ...
                <div className="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center mt-3 gap-2">
                  <div className="form-check d-flex align-items-center gap-1">
                    <input
                      ...
                      onChange={(e) => setFormData({ ...formData, showInCF4: e.target.checked })}
                      style={{
                  ...
                  >
                    This Text Field will hold maximum of 255 characters
                    <br className="d-none d-sm-block" />
                    <span style={{ color: "var(--primary, #0f763f)" }}>
                      Remaining available characters {remainingChars}
                    </span>
                  </div>
                </div>
              </div>
