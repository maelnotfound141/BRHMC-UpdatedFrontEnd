...
const PhysicalExamination = () => {
  ...
  // ─── Checkbox style: no disabled, use pointerEvents to block interaction ───
  const checkboxStyle = (borderColor: string = "var(--primary, #0f763f)"): React.CSSProperties => ({
    ...
    return (
      <div className="col-12 mt-4">
        <div className="form-check d-flex align-items-center gap-1 mb-2">
          {/* FIX: no disabled prop — use pointerEvents instead */}
          <input
          ...
          <label
            ...
            style={{ fontSize: "0.85rem", cursor: isUnlocked ? "pointer" : "default" }}
          >
            Other
          </label>
        </div>

        {/* FIX: readOnly instead of disabled — keeps white bg, no grey dimming */}
        <textarea
        ...
        <div className="d-flex flex-column flex-sm-row justify-content-between text-muted mt-2 gap-2" style={{ fontSize: "0.75rem", lineHeight: "1.4" }}>
          <span className="text-center text-sm-start">
            Date Logged <br className="d-none d-sm-block" />
      ...
      {options.map((opt, idx) => (
        <div className="col-12 col-sm-6 col-md-4 col-xl-3" key={idx}>
          <div className="form-check d-flex align-items-center gap-1">
            {/* FIX: no disabled prop — use pointerEvents to block interaction */}
            <input
              className="form-check-input mt-0 shadow-none"
              type="checkbox"
  ...
  const renderVitalSigns = () => {
    ...
    // FIX: always white bg, always full color — no grey when locked
    const getInputStyle = (borderColor: string, customWidth: string = "95px"): React.CSSProperties => ({
      ...
      <div className="d-flex flex-column gap-4 fade-in">
        <div className="d-flex flex-wrap align-items-start gap-4">
          ...
          <div className="d-flex flex-column justify-content-end">
            <div className="mb-2">
              ...
              <div className="form-check d-flex align-items-center mb-0 ms-1">
                {/* FIX: no disabled prop on palpatory checkbox */}
                <input
                  className="form-check-input mt-0 shadow-none"
                  type="checkbox"
                  id="vs-palpatory"
                  ...
                  style={{ ...checkboxStyle("#8e44ad"), border: "2px solid #8e44ad" }}
                />
                <label
                  className="form-check-label ms-1 pt-1 fw-semibold"
                  htmlFor="vs-palpatory"
                  style={{ fontSize: "0.8rem", color: "#8e44ad", cursor: isUnlocked ? "pointer" : "default" }}
                >
                  Palpatory
                </label>
              </div>
            ...
            <div className="d-flex gap-2">
              <div>
                {/* FIX: readOnly instead of disabled */}
                <input type="number" className="form-control shadow-none rounded-1" placeholder="0"
                  value={data.systolic}
                  onChange={(e) => updateField("Vital Sign", "systolic", e.target.value)}
                  readOnly={!isUnlocked}
                  style={getInputStyle("#8e44ad")} />
              ...
              <div>
                <input type="number" className="form-control shadow-none rounded-1" placeholder="0"
                  ...
                  onChange={(e) => updateField("Vital Sign", "diastolic", e.target.value)}
                  readOnly={!isUnlocked}
                  style={getInputStyle("#8e44ad")} />
          ...
          <div className="d-flex flex-wrap align-items-start gap-3 pb-2 vital-group-margin">
            <div>
              ...
              <input type="number" className="form-control shadow-none rounded-1" placeholder="0"
                value={data.hr}
                onChange={(e) => updateField("Vital Sign", "hr", e.target.value)}
                readOnly={!isUnlocked}
                style={getInputStyle("#17a2b8")} />
            </div>
            <div>
              <label style={getLabelStyle("#17a2b8")}>RR / Min.</label>
              <input type="number" className="form-control shadow-none rounded-1" placeholder="0"
                value={data.rr}
                onChange={(e) => updateField("Vital Sign", "rr", e.target.value)}
                readOnly={!isUnlocked}
                style={getInputStyle("#17a2b8")} />
            </div>
            <div>
              <label style={getLabelStyle("#17a2b8")}>Temperature</label>
              <input type="number" step="0.1" className="form-control shadow-none rounded-1" placeholder=".0"
                value={data.temp}
                onChange={(e) => updateField("Vital Sign", "temp", e.target.value)}
                readOnly={!isUnlocked}
                style={getInputStyle("#17a2b8", "140px")} />
            </div>
            <div>
              <label style={getLabelStyle("#17a2b8")}>O2 Saturation</label>
              <input type="number" step="0.01" className="form-control shadow-none rounded-1" placeholder=".00"
                value={data.o2}
                onChange={(e) => updateField("Vital Sign", "o2", e.target.value)}
                readOnly={!isUnlocked}
                style={getInputStyle("#17a2b8")} />
            </div>
          </div>
        ...
        <div className="d-flex flex-wrap align-items-start gap-4 mt-2">
          <div>
            ...
            <input type="number" step="0.01" className="form-control shadow-none rounded-1" placeholder=".00"
              value={data.height}
              onChange={(e) => updateField("Vital Sign", "height", e.target.value)}
              readOnly={!isUnlocked}
              style={getInputStyle("#333", "100px")} />
          </div>
          <div>
            <label style={getLabelStyle("#333")}>Weight (kg)</label>
            <input type="number" step="0.01" className="form-control shadow-none rounded-1" placeholder=".00"
              value={data.weight}
              onChange={(e) => updateField("Vital Sign", "weight", e.target.value)}
              readOnly={!isUnlocked}
              style={getInputStyle("#333", "100px")} />
          </div>
          <div>
            ...
            <input type="number" step="0.01" className="form-control shadow-none rounded-1" placeholder=".00"
              value={data.bmi}
              onChange={(e) => updateField("Vital Sign", "bmi", e.target.value)}
              readOnly={!isUnlocked}
              style={getInputStyle("#e67e22", "100px")} />
          </div>
        </div>
        ...
        {activeCategory === "Abdomen" && (
          <>
            ...
            <div className="mt-4 d-flex flex-wrap align-items-center gap-2">
              ...
              <input
                type="number"
                className="form-control shadow-none px-2 py-1 text-center"
                value={formData["Abdomen"].girth}
                onChange={(e) => updateField("Abdomen", "girth", e.target.value)}
                readOnly={!isUnlocked}
                style={{
                  width: "80px",
              ...
              >
                ...
                <div className="d-flex flex-column flex-grow-1 mb-4">
                  ...
                  <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-3 gap-3">
                    ...
                    <div
                      className={`d-flex flex-wrap justify-content-center justify-content-md-end pb-1 pb-md-0 ms-md-auto ${!catHasData && !isUnlocked ? "d-none d-lg-flex" : ""}`}
                      style={{ gap: "4px" }}
                    >
                      {/* Add — hidden once data exists or in edit mode */}
                      <button
                        onClick={handleAdd}
                        ...
                        className={`btn btn-sm border shadow-sm d-flex align-items-center justify-content-center gap-2 px-3 py-2 text-nowrap ${
                          (catHasData || isUnlocked) ? "bg-light text-muted opacity-50" : "text-white fw-bold"
                        }`}
                        style={{
                          borderRadius: "3px",
                          borderColor: "#0f763f",
                      ...
                      {/* Edit — hidden when no data or already in edit mode */}
                      <button
                        onClick={handleEdit}
                        disabled={!catHasData || isUnlocked}
                      ...
                      {/* Save — always visible, disabled when not in edit mode */}
                      <button
                        onClick={handleSave}
                        disabled={!isUnlocked}
                        className={`btn btn-sm border shadow-sm d-flex align-items-center justify-content-center gap-2 px-3 py-2 text-nowrap ${
                      ...
                      {/* Cancel — always visible, disabled when not in edit mode */}
                      <button
                      ...
                      {/* Delete — hidden when in edit mode or no data */}
                      <button
                      ...
                      >
                        ...
                        <span className="d-none d-md-inline">Del</span>
                      </button>
                    </div>
                  </div>
                        ...
                        {EXAM_CATEGORIES.map((category) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {hasContent && (
                                <i
                                  className={`isax isax-tick-circle ms-2 ${activeCategory === category ? "text-white" : "text-success"}`}
